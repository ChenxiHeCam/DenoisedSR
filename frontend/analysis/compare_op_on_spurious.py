"""
On the 29 high-R2-but-spurious cases: compare B (vars only) vs C (vars+ops).
Does the operator prior force PySR away from spurious formulas toward the truth?
"""
...
FEAT=74; HID=128; HEADS=4; SEED=42; Q=100; HELDOUT=300; N_DIST=20; TIMEOUT=10; OP_THRESH=0.03
...
def run_pysr(Xtr,ytr,Xte,yte,seed,bin_ops,un_ops):
    ...
    try:
        ...
        ss=np.sum((yte-yp)**2); tot=np.sum((yte-yte.mean())**2); return 1-ss/(tot+1e-12), str(m.sympy())
    except: return -99,"ERR"

# Spurious cases from previous analysis (R2 0.90-0.9999)
rows = json.loads(Path("data/results/numeric_equiv_rows.json").read_text())
spurious = [r["law_id"] for r in rows if 0.90 <= r["r2"] < 0.9999]
print(f"Re-testing {len(spurious)} spurious cases: B (all ops) vs C (predicted ops)\n")
...
b_better=0; c_better=0; same=0
print(f"{'Formula':24s} {'B_R2':>8s} {'C_R2':>8s} {'pred_unary':30s} {'verdict'}")
print("-"*95)
for lid in spurious:
    ...
    b_ops,un=predict_ops(aug,target)
    b_r2,_=run_pysr(Xtr,aug[target],Xte,te[target],SEED+i,['+','-','*','/','^'],ALL_UN)
    c_r2,_=run_pysr(Xtr,aug[target],Xte,te[target],SEED+i,b_ops,un)
    bE=b_r2>=0.9999; cE=c_r2>=0.9999
    if cE and not bE: verdict="C RESCUED!"; c_better+=1
    elif bE and not cE: verdict="C broke it"; b_better+=1
    else: verdict=""; same+=1
    print(f"{lid[:24]:24s} {b_r2:8.4f} {c_r2:8.4f} {','.join(un)[:30]:30s} {verdict}")

print("-"*95)
print(f"\nC rescued (spurious->exact): {c_better}")
print(f"C broke (exact->spurious):   {b_better}")
print(f"Same:                        {same}")
